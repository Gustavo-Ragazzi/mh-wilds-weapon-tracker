...
class WeaponTracker:
    ...
    def get_next_weapons(self) -> Tuple[Weapon, Weapon]:
        primary = next(
            w for w in self.weapons.values() if w.primary == self.get_min("primary")
        )
        secondary = next(
            w
            for w in self.weapons.values()
            if (w.secondary - w.adjustment) == self.get_min("secondary_with_adjustment")
        )
    ...
    def quests_remaining(self) -> Tuple[int, int]:
        ...
        expected_secondary = {
            w.name: w.primary + w.adjustment for w in self.weapons.values()
        }
        max_secondary = max(expected_secondary.values())
        secondary_remain = sum(
            max_secondary - (w.secondary - w.adjustment) for w in self.weapons.values()
        )
